...
Institution-specific catalogues are loaded automatically from `data/*_academic_catalog.json`. The current verified set covers Bayero University Kano, Ahmadu Bello University Zaria, and Abubakar Tafawa Balewa University Bauchi. Each file records its official sources and verification date. When a covered university is selected, registration and course creation use its own faculty, department, and programme hierarchy instead of the generic NUC discipline baseline.
